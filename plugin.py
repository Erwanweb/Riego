...
# V2.3.6 : force an Off command on the other zone valves on every zone change instead of trusting the state cache (fixes multiple zone valves staying active at once)
# V2.3.7 : when stopping (incl. Off mode), force Off + verify real device state of ALL valves (zones + general valve) via Domoticz readback, retrying once if not confirmed Off
# V2.3.8 : periodic idle safety check every 10 min (outside watering periods only) re-forcing/verifying all valves Off, incl. general valve; never runs while a cycle is active so the general valve stays open during watering
# V2.3.9 : Test zone duration reduced to 30s (was 1 min), heartbeat interval reduced to 15s (was 20s); Manual zone duration unchanged at 1 min
# V2.3.10 : fixed idle periodic check firing immediately right after the startup safety phase (redundant extra Off burst on all valves); it now waits its full 10-minute interval after startup like any other cycle
#
# Version: 2.3.10

"""
<plugin key="ZZ-AIS7Z" name="RONELABS - Auto Irrigation Sys" author="ErwanBCN" version="2.3.10" externallink="https://ronelabs.com">
    <description>
        <h2>Automatic Irrigation System V2.3.10</h2><br/>
...
TEST_ZONE_DURATION_SECONDS = 30

STARTUP_SAFETY_SECONDS = 60
IDLE_VALVE_CHECK_SECONDS = 600  # contrôle périodique "toutes vannes Off" hors période d'arrosage
...
class BasePlugin:
    def __init__(self):
        ...
        self.next_idle_valve_check = None

        self._device_state_cache = {}

    def onStart(self):
        Domoticz.Log("RONELABS Irrigation V2.3.10: onStart called")
    ...
    def onHeartbeat(self):
        ...
        if self.startup_phase:
            ...
            if now >= self.startup_end:
                ...
                self.startup_phase = False
                # Le "startup final" ci-dessus vient déjà de forcer/vérifier toutes les
                # vannes Off : on amorce le minuteur ici pour que le contrôle périodique
                # idle attende son plein intervalle avant sa première exécution, au lieu
                # de se redéclencher immédiatement au heartbeat suivant.
                self.next_idle_valve_check = now + timedelta(seconds=IDLE_VALVE_CHECK_SECONDS)
        ...
        if not self.run_active:
            if self.next_idle_valve_check is None or now >= self.next_idle_valve_check:
                self._verify_idle_valves_off()
                self.next_idle_valve_check = now + timedelta(seconds=IDLE_VALVE_CHECK_SECONDS)
    ...
    def _handle_control_command(self, level):
        ...
        if level == MODE_TEST:
            ...
            Domoticz.Log(f"Irrigation mode: Test - starting {TEST_ZONE_DURATION_SECONDS}s per zone")
    ...
    def _start_current_zone(self, now):
        ...
        duration = (TEST_ZONE_DURATION_SECONDS / 60.0) if self.run_type == "test" else self.zone_minutes[self.current_zone_index]
    ...
    def _open_only_zone(self, zone_index):
        ...
        other_zone_idxs = [idx for idx in self.zone_idxs if idx != target_idx]
        self._force_valves_off(other_zone_idxs)

    def _close_zones_only(self):
        self._force_valves_off(self.zone_idxs)

    def _force_valves_off(self, idxs):
        """
        Envoie systématiquement la commande Off (sans se fier au cache d'état)
        à chaque changement de zone : le cache pouvait indiquer "Off" pour une
        vanne alors qu'elle était réellement restée active (commande précédente
        perdue, état changé hors du plugin, etc.), ce qui laissait plusieurs
        vannes de zone actives en même temps.
        """
        for idx in idxs:
            if not idx:
                continue

            res = DomoticzAPI(f"type=command&param=switchlight&idx={idx}&switchcmd=Off")

            if not res or str(res.get("status", "")).lower() != "ok":
                Domoticz.Error(f"Forced Off failed for zone valve idx={idx}")
                continue

            self._device_state_cache[idx] = "Off"

    def _all_valves_off(self):
        all_idxs = self.zone_idxs + self.main_valve_idxs

        self._force_valves_off(all_idxs)
        self._verify_all_valves_off(all_idxs)

    def _verify_idle_valves_off(self):
        """
        Contrôle périodique (toutes les IDLE_VALVE_CHECK_SECONDS) exécuté
        uniquement hors période d'arrosage (self.run_active == False) : force
        et vérifie que toutes les vannes, y compris la vanne générale, sont
        bien Off. Ne doit jamais tourner pendant un cycle actif, sous peine de
        fermer la vanne générale alors qu'elle doit rester ouverte.
        """
        if self.debug:
            Domoticz.Debug("Idle periodic check: verifying all valves are Off")

        self._all_valves_off()

    def _verify_all_valves_off(self, idxs, retries=1):
        """
        Après un Off, relit l'état réel de chaque vanne (zones + vanne générale)
        auprès de Domoticz au lieu de se fier à la seule réponse de la commande :
        si une vanne n'est pas confirmée Off, on retente l'envoi de la commande
        avant de logger une erreur.
        """
        remaining = [idx for idx in idxs if idx]
        attempt = 0

        while remaining and attempt <= retries:
            attempt += 1
            still_on = [idx for idx in remaining if self._read_switch_state_from_domoticz(idx) != "Off"]

            for idx in remaining:
                if idx not in still_on:
                    self._device_state_cache[idx] = "Off"

            if still_on and attempt <= retries:
                Domoticz.Log(f"Valve(s) not confirmed Off after stop, retrying: idx={still_on}")
                self._force_valves_off(still_on)

            remaining = still_on

        if remaining:
            Domoticz.Error(f"Valve(s) could not be confirmed Off after stop: idx={remaining}")
    ...
    def _remaining_total_minutes(self, now=None):
        ...
        if self.run_type == "test":
            if self.current_zone_index is not None:
                total += max(0, 6 - self.current_zone_index) * (TEST_ZONE_DURATION_SECONDS / 60.0)
